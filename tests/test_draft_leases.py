...
def test_retrieve_lease():
    ...
    assert lease_id
    lease_rep = dl.draft_leases.retrieve(lease_id)
...
def test_list_lease_tenants():
    lease_resp = dl.draft_leases.list_lease_tenants()
...
def test_list_lease_tenants_filter_lease():
    ...
    lease_id = response['data'][0].get('id')
    assert lease_id
    lease_resp = dl.draft_leases.list_lease_tenants(query_params={
